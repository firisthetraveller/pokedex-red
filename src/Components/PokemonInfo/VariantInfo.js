        ...
        <>
            {
                displayedData.length > 0
                    ? displayedData.map((d, i) => <div key={i} className="m-2 w-40">
                        <Link to={`/pokemon/${d.pokemon.name}`} className="flex flex-col items-center">
                            <img src={getImageFromName(d.pokemon.name)}  className="size-24 lg:size-32" alt={`Sprite for ${capitalizeAllString(d.pokemon.name)}`} />
